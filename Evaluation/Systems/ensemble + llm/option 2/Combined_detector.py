...
user_flags = merged.groupby('user')['ensemble_OR'].max().reset_index()
ensemble_flagged = set(user_flags[user_flags['ensemble_OR']]['user'].astype(str).unique())
...
print(f"\n--- METHOD BREAKDOWN ---")
print(f"Ensemble OR flagged: {len(ensemble_flagged)}")
...
print(f"\nCombined (Ensemble OR OR LLM): {len(all_flagged)}")
...
print(f"Ensemble OR alone:    {ensemble_recall:.1f}% recall")
